...
namespace SQLCover
{
    public class CoverageResult : CoverageSummary
    {
        ...
        public string RawXml()
        {
            ...
            foreach (var batch in _batches)
            {
                builder.AppendFormat("<Batch Object=\"{0}\" StatementCount=\"{1}\" CoveredStatementCount=\"{2}\">", SecurityElement.Escape(batch.ObjectName), batch.StatementCount, batch.CoveredStatementCount);
                builder.AppendFormat("<Text>\r\n![CDATA[{0}]]</Text>", XmlTextEncoder.Encode(batch.Text));
                foreach (var statement in batch.Statements)
                {
                    builder.AppendFormat("\t<Statement HitCount=\"{0}\" Offset=\"{1}\" Length=\"{2}\" CanBeCovered=\"{3}\"></Statement>", statement.HitCount, statement.Offset, statement.Length, statement.IsCoverable);
                }
        ...
        public string Html()
        {
            ...
            builder.Append("<table><thead><td>object name</td><td>statement count</td><td>covered statement count</td><td>coverage %</td></thead>");

            builder.AppendFormat("<tr><td>{0}</td><td>{1}</td><td>{2}</td><td>{3:0.00}</td></tr>", "<b>Total</b>", statements, coveredStatements, (float) coveredStatements/(float) statements*100.0);

            foreach (var batch in _batches.Where(p=>!p.ObjectName.Contains("tSQLt")).OrderByDescending(p => (float) p.CoveredStatementCount/(float) p.StatementCount))
            {
                builder.AppendFormat("<tr><td><a href=\"#{0}\">{0}</a></td><td>{1}</td><td>{2}</td><td>{3:0.00}</td></tr>", batch.ObjectName, batch.StatementCount, batch.CoveredStatementCount, (float) batch.CoveredStatementCount/(float) batch.StatementCount*100.0);
            }
        ...
        public string OpenCoverXml()
        {
            ...
            builder.AppendFormat("<CoverageSession xmlns:xsd=\"http://www.w3.org/2001/XMLSchema\" xmlns:xsi=\"http://www.w3.org/2001/XMLSchema-instance\">" +
            ...
            foreach (var batch in _batches)
            {
                builder.AppendFormat("<Class><Summary numSequencePoints=\"{0}\" visitedSequencePoints=\"{1}\" numBranchPoints=\"0\" visitedBranchPoints=\"0\" sequenceCoverage=\"{2}\" branchCoverage=\"0\" maxCyclomaticComplexity=\"0\" minCyclomaticComplexity=\"0\" /><FullName>{3}</FullName><Methods>"
                    , batch.Statements.Count, batch.CoveredStatementCount, (float) batch.CoveredStatementCount/(float) batch.StatementCount*100.0
                    , batch.ObjectName);


                builder.AppendFormat("\t\t<Method visited=\"{0}\" cyclomaticComplexity=\"0\" sequenceCoverage=\"{1}\" branchCoverage=\"0\" isConstructor=\"false\" isStatic=\"false\" isGetter=\"true\" isSetter=\"false\">\r\n",
                    batch.CoveredStatementCount > 0 ? "true" : "false",
                    batch.CoveredStatementCount/(float) batch.StatementCount*100.0);

                builder.AppendFormat("\t\t<Summary numSequencePoints=\"{1}\" visitedSequencePoints=\"0\" numBranchPoints=\"0\" visitedBranchPoints=\"0\" sequenceCoverage=\"{2}\" branchCoverage=\"0\" maxCyclomaticComplexity=\"0\" minCyclomaticComplexity=\"0\" />\r\n",
                    batch.StatementCount, batch.CoveredStatementCount, batch.CoveredStatementCount/(float) batch.StatementCount*100.0);


                builder.AppendFormat("\t\t<MetadataToken>01041980</MetadataToken><Name>{0}</Name><FileRef uid=\"{1}\" />\r\n", batch.ObjectName, fileMap[batch.ObjectName]);
                ...
                foreach (var statement in batch.Statements)
                {
                   ...
                   // {
                        ...
                        builder.AppendFormat("\t\t\t<SequencePoint vc=\"{0}\" uspid=\"{1}\" ordinal=\"{2}\" offset=\"{3}\" sl=\"{4}\" sc=\"{5}\" el=\"{6}\" ec=\"{7}\" />\r\n",
                            statement.HitCount
                            , i++
                            , j++
                            , statement.Offset
